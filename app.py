...
class Order(db.Model):
    ...
    def order_total(self):
        return db.session\
            .query(db.func.sum(Order_Item.quantity * Product.price))\
            .join(Product)\
    ...
    def quantity_total(self):
        return db.session\
            .query(db.func.sum(Order_Item.quantity))\
            .filter(Order_Item.order_id == self.id)\
            .scalar()
...
def add():
    ...
    if form.validate_on_submit():
        ...
        file.save(path)
        image_url = os.path.join('/static/imgs/', secure_filename(file.filename))
